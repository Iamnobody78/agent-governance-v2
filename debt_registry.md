...
| DEBT-0026 | json_path 规则线性匹配（规则多时逐条解析 jsonpath 遍历，命中率低；→ 前缀索引树 ~60 行） | LOW | 2026-08-03 | 否 | 暗雷区 P3（已批准按序执行，待启动） |

> **当前活跃债务：4 项（MEDIUM×1 / LOW×3，无阻塞）** —— 暗雷区修复 P0-P2 closeout 时清点。DEBT-0023（P0 异常日志）、DEBT-0024（P1 语义钩子）、DEBT-0025（P2 SQLite 写锁）已清偿（见清偿表）；DEBT-0026（json_path 线性匹配）为暗雷区 P3，待执行。DEBT-0022 已清偿（`6c25bd9`，REAL-011.1）；DEBT-0019 已清偿（`d95f83c`）。上一批 16/16 已清偿于 TASK-REAL-001..008。
